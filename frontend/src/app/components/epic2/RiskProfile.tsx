...
import {
  ...
  Building2, FileText, TrendingDown, X
} from 'lucide-react';
...
function MetricCard({ icon: Icon, label, value, unit, color, danger }: {
  icon: React.ElementType; label: string; value: number | string; unit?: string; color: string; danger?: boolean;
}) {
  return (
    <div className={`rounded-xl p-3 border ${
      danger ? 'bg-red-50/50 border-red-100' : 'bg-white border-slate-100'
    }`}>
      <div className="flex items-center gap-2 mb-2">
        <div className={`p-1.5 rounded-lg ${danger ? 'bg-red-100' : 'bg-slate-50'}`}>
          <Icon size={13} className={danger ? 'text-red-600' : 'text-slate-500'} />
        </div>
  ...
  if (!supplier) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
        ...
        <h3 className="text-slate-700 mb-1 font-medium">No Supplier Selected</h3>
      ...
      >
          ...
          <div className="flex items-start justify-between mb-3">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[11px] text-slate-400 font-mono">{supplier.id}</span>
                <span className={`px-2 py-0.5 rounded-full text-[10px] capitalize ${colors.light} ${colors.text}`}>
                  {supplier.riskLevel} risk
                </span>
              </div>
              <h2 className="text-slate-800 font-semibold tracking-tight">{supplier.name}</h2>
            </div>
          ...
          <div className="mt-4 flex items-center gap-4">
            <div className="relative w-16 h-16">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[18px] font-semibold" style={{ color: colors.dot }}>
                  {supplier.riskScore}
                </span>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div className="grid grid-cols-2 gap-2">
            <MetricCard icon={Shield} label="Protected Overlap" value={supplier.protectedAreaOverlap} unit="%"
              color={supplier.protectedAreaOverlap > 30 ? '#dc2626' : '#10b981'}
              danger={supplier.protectedAreaOverlap > 30} />
            <MetricCard icon={AlertTriangle} label="Threatened Spp." value={supplier.threatenedSpeciesCount} unit="species"
              color={supplier.threatenedSpeciesCount > 20 ? '#dc2626' : '#f59e0b'}
              danger={supplier.threatenedSpeciesCount > 20} />
            <MetricCard icon={TrendingDown} label="Deforestation" value={supplier.deforestationRate} unit="% /yr"
              color={supplier.deforestationRate > 1.5 ? '#dc2626' : '#f59e0b'}
              danger={supplier.deforestationRate > 1.5} />
            <MetricCard icon={Leaf} label="Carbon Stock" value={supplier.carbonStock} unit="t/ha" color="#10b981" />
          </div>

          {/* Threatened Species */}
          <div>
            <h4 className="text-[12px] text-slate-500 uppercase tracking-wider mb-2">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] text-slate-800 truncate">{species.name}</div>
                      <div className="text-[10px] text-slate-400 capitalize">{species.type}</div>
                    </div>
          ...
          {supplier.notes && (
            <div className="p-3 bg-white rounded-xl border border-slate-100">
              ...
              <p className="text-[12px] text-slate-700 leading-relaxed">{supplier.notes}</p>
            </div>
          )}
          ...
          <div className="p-3 bg-white rounded-xl border border-slate-100">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[13px] text-slate-800 font-semibold min-w-[36px] text-right">
