...
import { Search, Filter, ChevronDown, MapPin, AlertTriangle, Shield, TreePine } from 'lucide-react';
...
export default function SupplierList({ suppliers, selectedId, onSelect, hoveredId, onHover }: SupplierListProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-earth-50">
      {/* Header */}
      <div className="p-4 border-b border-earth-200 bg-white">
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="text-forest-800 tracking-tight">Suppliers</h2>
            <p className="text-[13px] text-earth-500 mt-0.5">{suppliers.length} locations monitored</p>
          </div>
          <button
            onClick={() => setShowFilter(!showFilter)}
            className={`p-2 rounded-lg transition-colors ${showFilter ? 'bg-forest-100 text-forest-700' : 'text-earth-500 hover:bg-earth-100'}`}
          >
            <Filter size={16} />
          </button>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-earth-50 border border-earth-200 rounded-lg text-[13px] text-earth-800 placeholder:text-earth-400 focus:outline-none focus:border-forest-400 focus:ring-1 focus:ring-forest-400/30 transition-all"
          />
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1.5">
          ...
          return (
            <motion.button
              ...
              className={`w-full text-left p-3 rounded-xl transition-all cursor-pointer ${
                isSelected
                  ? 'bg-white shadow-md ring-1 ring-forest-300/50'
                  : isHovered
                  ? 'bg-white shadow-sm'
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    />
                    <h4 className="text-[13px] text-earth-900 truncate font-[family-name:var(--font-family-sans)]">
                      {supplier.name}
                    </h4>
                  </div>
                  <div className="flex items-center gap-1 ml-4 text-[11px] text-earth-500">
                    <MapPin size={10} />
                    {supplier.region}
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3 mt-2.5 ml-4">
                <div className="flex items-center gap-1 text-[10px] text-earth-500">
                  <Shield size={10} className="text-earth-400" />
                  {supplier.protectedAreaOverlap}% overlap
                </div>
                <div className="flex items-center gap-1 text-[10px] text-earth-500">
                  <AlertTriangle size={10} className="text-earth-400" />
                  {supplier.threatenedSpeciesCount} species
                </div>
                <div className="flex items-center gap-1 text-[10px] text-earth-500">
                  <TreePine size={10} className="text-earth-400" />
                  {supplier.vegetationCondition}%
                </div>
              </div>

              {/* Risk bar */}
              <div className="mt-2.5 ml-4 h-1 bg-earth-100 rounded-full overflow-hidden">
                <motion.div
        ...
        {sortedSuppliers.length === 0 && (
          <div className="text-center py-10 text-earth-400 text-[13px]">
